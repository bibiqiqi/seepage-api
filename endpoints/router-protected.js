...
const mongoConn = mongoose.connection;
  ...
  mongoConn.once("open", () => {
    ...
    function successResponse(res, results) {
    ...
    function insertUrlSubDoc(contentId, url) {
      //console.log('calling findByIdAndUpdate to insert url subdoc');
      return new Promise(function(resolve, reject) {
        Content.
        findByIdAndUpdate(
          contentId,
    ...
    // async function uploadSubDocs(files, contentId) {
    //   //console.log('files passed to uploadSubDocs is', files)
    //   files.map(file => {
    //     await uploadSubDoc(file, contentId)
    //   })
    // }
        ...
        .then(insertedContent => {
          ...
          return uploadSubDocs(files, insertedContent.id) //resolves with the complete doc
            .then(() => {
              ...
              Content.findById(insertedContent.id, function (err, contentDoc) {
                if(!err) {
                  successResponse(res, contentDoc);
                }
              })
            })
            .catch((err) => {
    ...
    function remSubDocsAndFiles(subDocIdsArray, contentId) {
      return new Promise(function(resolve, reject) {
        Content.findById(contentId, function (err, contentDoc) {
          ...
          let removeFromGfs = []; //making an array of file Ids to query gfs with
...
module.exports = { router, mongoConn };
